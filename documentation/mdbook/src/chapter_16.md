...
The last major change to the `GUI` for a combined environments test is the inclusion of the `Test Profile` tab which allows users to create a timeline of events that will occur in the combined environments test.  This is described more thoroughly in Section \ref{sec:using_rattlesnake_test_profiles}.
