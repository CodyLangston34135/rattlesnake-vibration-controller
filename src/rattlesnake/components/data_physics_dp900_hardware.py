...
class DataPhysicsDP900Acquisition(HardwareAcquisition):
    ...
    def set_up_data_acquisition_parameters_and_channels(self,
        ...
        if self.dp900.status == self.DP900Status.RUNNING:
            self.dp900.stop()
        if self.dp900.status == self.DP900Status.STOPPED or self.dp900.status == self.DP900Status.INIT:
            self.dp900.end()
        ...
        for ct_index, channel in enumerate(channel_data):
            ...
            if not system in system_list:
                raise ValueError('System {:} is not a valid system.  Must be one of {:}'.format(system,system_list))
            if not system in systems:
                systems.append(system)
                if len(systems) > 1:
                    raise ValueError('Multi-chassis tests are not currently supported in Rattlesnake')
            ...
            if is_output:
                ...
                if len(bnc_index) > 1:
                    raise ValueError('More than one matching channel for BNC {:} (how did this happen?)'.format(channel.physical_channel))
                if len(bnc_index) < 1:
                    raise ValueError('BNC {:} was not found in the list of output BNCs {:}.  Please run DP900Config to correctly set input and output channels.'.format(
                ...
                if len(bnc_index) > 1:
                    raise ValueError('More than one matching channel for BNC {:} (how did this happen?)'.format(channel.physical_channel))
                if len(bnc_index) < 1:
                    raise ValueError('BNC {:} was not found in the list of input BNCs {:}.  Please run DP900Config to correctly set input and output channels.'.format(
    ...
    def read(self) -> np.ndarray:
        ...
        while self.dp900.get_available_input_data_samples() < self.data_acquisition_parameters.samples_per_read:
