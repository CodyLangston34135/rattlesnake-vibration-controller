...
class SpectralProcessingProcess(AbstractMessageProcess):
    ...
    def run_spectral_processing(self, data):
        ...
        if self.spectral_processing_parameters.averaging_type == AveragingTypes.LINEAR:
            ...
            self.log(
                "Buffered Frames (Resp Shape: {:}, Ref Shape: {:})".format(
                    self.response_fft.shape, self.reference_fft.shape
                )
            )
            ...
            if np.all(exclude_averages):
                ...
                return
            self.log(
                "Mean FFT Value Over Averaged Frames: \n  {:}".format(
                    np.mean(
            ...
            if (
                ...
                self.log(
                    "Computed Response Spectral Matrix in {:0.2f} seconds".format(
                        time.time() - response_spectral_time
                    )
                )
            ...
            if (
                ...
                self.log(
                    "Computed Reference Spectral Matrix in {:0.2f} seconds".format(
                        time.time() - reference_spectral_time
                    )
                )
            ...
            if self.spectral_processing_parameters.requires_spectral_reference_response:
                ...
                self.log(
                    "Computed Crossspectral Matrix in {:0.2f} seconds".format(
                        time.time() - cross_spectral_time
                    )
                )
        ...
        self.log(
            "Computed Spectral Matrices for {:} frames in {:0.2f} seconds".format(
                frames, time.time() - response_spectral_time
            )
        )
        Gffpinv = None
        Gfxpinv = None
        if self.spectral_processing_parameters.compute_frf:
            ...
            self.log(
                "Computed FRF Condition Number in {:0.2f} seconds".format(
                    time.time() - cond_time
                )
            )
        else:
        ...
        if self.spectral_processing_parameters.compute_coherence:
            ...
            self.log(
                "Computed Coherence in {:0.2f} seconds".format(time.time() - coh_time)
            )
        else:
            coherence = None
        if self.spectral_processing_parameters.compute_cpsd:
            ...
            response_spectral_matrix *= (
                self.spectral_processing_parameters.frequency_spacing  # Window correction was done in the data collector
            ...
            self.log(
                "Computed CPSDs in {:0.2f} seconds".format(time.time() - cpsd_time)
            )
            ...
            reference_spectral_matrix[1:-1] *= 2
            self.log(
                "Computed APSDs in {:0.2f} seconds".format(time.time() - apsd_time)
            )
        else:
