...
from rattlesnake.environment.abstract_environment import EnvironmentProcess, EnvironmentMetadata
...
from rattlesnake.process.spectral import (
    AveragingTypes,
    Estimator,
...
# region: Metadata
class AbstractSysIdMetadata(EnvironmentMetadata):
...
# region: Process
class AbstractSysIdEnvironment(EnvironmentProcess):
    ...
    def initialize_environment(self, metadata: AbstractSysIdMetadata):
        """
    ...
    def get_sysid_data_collector_metadata(self) -> CollectorMetadata:
        ...
        if self.environment_metadata.sysid_signal_type in [
            "Random",
            "Pseudorandom",
            "Chirp",
        ]:
        ...
        if self.environment_metadata.sysid_signal_type == "Random":
            overlap_fraction = self.environment_metadata.sysid_overlap
        else:
            overlap_fraction = 0
        if self.environment_metadata.sysid_signal_type == "Burst Random":
            trigger_channel_index = reference_channel_indices[0]
        else:
            trigger_channel_index = 0
        trigger_slope = TriggerSlope.POSITIVE
        trigger_level = self.environment_metadata.sysid_level / 100
        trigger_hysteresis = self.environment_metadata.sysid_level / 200
        trigger_hysteresis_samples = ((1 - self.environment_metadata.sysid_burst_on) * self.environment_metadata.sysid_frame_size) // 2
        pretrigger_fraction = self.environment_metadata.sysid_pretrigger
        frame_size = self.environment_metadata.sysid_frame_size
        window = Window.HANN if self.environment_metadata.sysid_window == "Hann" else Window.RECTANGLE
        kurtosis_buffer_length = self.environment_metadata.sysid_averages
    ...
    def get_sysid_spectral_processing_metadata(self, is_noise=False) -> SpectralProcessingMetadata:
        ...
        averaging_type = AveragingTypes.LINEAR if self.environment_metadata.sysid_averaging_type == "Linear" else AveragingTypes.EXPONENTIAL
        averages = self.environment_metadata.sysid_noise_averages if is_noise else self.environment_metadata.sysid_averages
        exponential_averaging_coefficient = self.environment_metadata.sysid_exponential_averaging_coefficient
        if self.environment_metadata.sysid_estimator == "H1":
            frf_estimator = Estimator.H1
        elif self.environment_metadata.sysid_estimator == "H2":
            frf_estimator = Estimator.H2
        elif self.environment_metadata.sysid_estimator == "H3":
            frf_estimator = Estimator.H3
        elif self.environment_metadata.sysid_estimator == "Hv":
            frf_estimator = Estimator.HV
        else:
            raise ValueError(f"Invalid FRF Estimator {self.environment_metadata.sysid_estimator}")
        ...
        frequency_spacing = self.environment_metadata.sysid_frequency_spacing
        sample_rate = self.environment_metadata.sample_rate
        num_frequency_lines = self.environment_metadata.sysid_fft_lines
    ...
    def get_sysid_signal_generation_metadata(self) -> SignalGenerationMetadata:
        ...
        return SignalGenerationMetadata(
            ...
            level_ramp_samples=self.environment_metadata.sysid_level_ramp_time
    ...
    def get_sysid_signal_generator(self) -> SignalGenerator:
        ...
        if self.environment_metadata.sysid_signal_type == "Random":
            return RandomSignalGenerator(
                rms=self.environment_metadata.sysid_level,
                sample_rate=self.environment_metadata.sample_rate,
                num_samples_per_frame=self.environment_metadata.sysid_frame_size,
                num_signals=self.environment_metadata.num_reference_channels,
                low_frequency_cutoff=self.environment_metadata.sysid_low_frequency_cutoff,
                high_frequency_cutoff=self.environment_metadata.sysid_high_frequency_cutoff,
                cola_overlap=0.5,
                cola_window="hann",
                cola_exponent=0.5,
                output_oversample=self.hardware_metadata.output_oversample,
            )
        elif self.environment_metadata.sysid_signal_type == "Pseudorandom":
        ...
        elif self.environment_metadata.sysid_signal_type == "Burst Random":
            return BurstRandomSignalGenerator(
                ...
                low_frequency_cutoff=self.environment_metadata.sysid_low_frequency_cutoff,
                high_frequency_cutoff=self.environment_metadata.sysid_high_frequency_cutoff,
                on_fraction=self.environment_metadata.sysid_burst_on,
                ramp_fraction=self.environment_metadata.sysid_burst_ramp_fraction,
                output_oversample=self.hardware_metadata.output_oversample,
            )
        elif self.environment_metadata.sysid_signal_type == "Chirp":
            return ChirpSignalGenerator(
                level=self.environment_metadata.sysid_level,
                sample_rate=self.environment_metadata.sample_rate,
                num_samples_per_frame=self.environment_metadata.sysid_frame_size,
                num_signals=self.environment_metadata.num_reference_channels,
                low_frequency_cutoff=np.max(
                    [
                        ...
                        self.environment_metadata.sysid_low_frequency_cutoff,
                    ]
                ),
                high_frequency_cutoff=np.min(
                    [
                        self.environment_metadata.sample_rate / 2,
                        self.environment_metadata.sysid_high_frequency_cutoff,
                    ]
                ),
                output_oversample=self.hardware_metadata.output_oversample,
            )
    ...
    def preview_noise(self, data):
        ...
        self.collector_command_queue.put(
            ...
            (
                ...
                (self.environment_metadata.sysid_skip_frames, 1),
            ),
        )
        time.sleep(0.01)
        ...
        self.data_analysis_command_queue.put(self.environment_name, (SysIDDataAnalysisCommands.RUN_NOISE, False))
    ...
    def start_noise(self, data):
        ...
        self.spectral_shutdown_achieved = False
        self.analysis_shutdown_achieved = False
        self.environment_metadata, self._sysid_stream_name = data
        self.controller_communication_queue.put(
            self.environment_name,
            (
    ...
    def stop_system_id(self, stop_tasks):
        ...
        self.collector_command_queue.put(
            ...
            (
                DataCollectorCommands.SET_TEST_LEVEL,
                (self.environment_metadata.sysid_skip_frames * 10, 1),
            ),
        )
    ...
    def check_for_sysid_shutdown(self, data):  # pylint: disable=unused-argument
        ...
        if (
            ...
            if self._waiting_to_start_transfer_function:
                self.start_transfer_function(self.environment_metadata)
            else:
