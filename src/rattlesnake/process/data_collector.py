...
from rattlesnake.utilities import VerboseMessageQueue, flush_queue, load_python_module, rms_time
import copy
...
class FrameBuffer:
    ...
    def add_data(self, data):
        ...
        self.buffer_data[:] = np.concatenate((self.buffer_data[..., data.shape[-1] :], data), axis=-1)
    ...
    def find_triggers(self):
        ...
        if self.manual_accept and self.waiting_for_accept:
            ...
            return []
        if self.trigger_enabled and ((self.trigger_only_first and self.first_trigger) or not self.trigger_only_first):
            ...
            resets = np.concatenate(
                [
                    [self.last_reset],
                    self.buffer_size_frame_multiplier * self.samples_per_frame - reset_indices - self.pretrigger_samples,
                ]
            )
...
class DataCollectorProcess(AbstractMessageProcess):
    ...
    def force_initialize_collector(self, data: CollectorMetadata):
        ...
        if self.collector_metadata.window == Window.RECTANGLE:
            ...
            self.reference_window = sig.get_window("blackmanharris", self.collector_metadata.frame_size)
            self.response_window = self.reference_window.copy()
    ...
    def acquire(self, data):  # pylint: disable=unused-argument
        ...
        if output_frames.shape[0] > 0:
            ...
            for frame in output_frames:
                ...
                self.log(f"Received output from framebuffer with RMS: \n  " f"{rms_time(reference_frame, axis=-1)}")
    ...
    def set_test_level(self, data):
        ...
        self.log(f"Setting Test Level to {self.test_level}, skipping next {self.skip_frames} frames")
