...
from rattlesnake.process.signal_generation_utilities import SignalGenerator
...
class SignalGenerationProcess(AbstractMessageProcess):
    ...
    def generate_signals(self, data):
                    ...
                    self.gui_update_queue.put(
                        (
                            ...
                            (
                                ...
                                f"{self.process_name} timed out while waiting for first " "set of parameters",
                            ),
                        )
                    )
                    return
        ...
        if (
            (self.signal_remainder is None or self.signal_remainder.shape[-1] < self.new_signal_sample_threshold)
        ...
        if self.data_out_queue.empty() and self.signal_remainder is not None and self.signal_remainder.shape[-1] > 0:
            ...
            last_run = (self.shutdown_flag and self.current_test_level == 0.0) or (self.done_generating and self.signal_remainder.shape[-1] == 0)
    ...
    def adjust_test_level(self, data):
        ...
        if self.test_level_change != 0.0:
            self.log(f"Changed test level from {self.current_test_level} to {self.test_level_target}, " f"{self.test_level_change} change per sample")
