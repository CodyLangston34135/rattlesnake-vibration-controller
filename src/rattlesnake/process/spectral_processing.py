...
class SpectralProcessingMetadata:
    ...
    def requires_full_spectral_response(self):
        ...
        if (self.compute_frf and self.frf_estimator in [Estimator.H2, Estimator.H3]) or self.compute_cpsd:
            return True
        else:
            return False
    ...
    def requires_diagonal_spectral_response(self):
        ...
        if (self.compute_frf and self.frf_estimator in [Estimator.HV]) or self.compute_apsd or self.compute_coherence:
            return True
        else:
            return False
    ...
    def requires_full_spectral_reference(self):
        ...
        if (self.compute_frf and self.frf_estimator in [Estimator.H1, Estimator.H3, Estimator.HV]) or self.compute_cpsd or self.compute_coherence:
            return True
        else:
            return False
...
class SpectralProcessingProcess(AbstractMessageProcess):
    ...
    def initialize_parameters(self, data: SpectralProcessingMetadata):
        ...
        if self.spectral_processing_parameters is None:
            ...
            or self.spectral_processing_parameters.num_response_channels != data.num_response_channels
            or self.spectral_processing_parameters.num_reference_channels != data.num_reference_channels
    ...
    def run_spectral_processing(self, data):
        ...
        if self.spectral_processing_parameters.averaging_type == AveragingTypes.LINEAR:
            ...
            self.log(f"Buffered Frames (Resp Shape: {self.response_fft.shape}, " f"Ref Shape: {self.reference_fft.shape})")
            ...
            if self.spectral_processing_parameters.requires_full_spectral_response:
                ...
                self.response_diagonal_matrix = np.mean(
                    self.response_fft[~exclude_averages] * np.conj(self.response_fft[~exclude_averages]),
                    axis=0,
                ).T
            if (
                ...
                self.log("Computed Response Spectral Matrix in " f"{time.time() - response_spectral_time:0.2f} seconds")
            ...
            if self.spectral_processing_parameters.requires_full_spectral_reference:
                ...
                # Get the diagonal matrix as well
                self.reference_diagonal_matrix = np.einsum("fii->fi", self.reference_spectral_matrix)
            ...
            if (
                ...
                self.log(f"Computed Reference Spectral Matrix in " f"{time.time() - reference_spectral_time:0.2f} seconds")
            ...
            if self.spectral_processing_parameters.requires_spectral_reference_response:
                ...
                self.log("Computed Crossspectral Matrix in " f"{time.time() - cross_spectral_time:0.2f} seconds")
                    ...
                    if self.response_spectral_matrix is None:
                        ...
                        self.response_spectral_matrix = (
                            ...
                            * np.einsum("if,jf->fij", response_fft, np.conj(response_fft))
                            + (1 - self.spectral_processing_parameters.exponential_averaging_coefficient) * self.response_spectral_matrix
                        )
                    # Get the diagonal matrix as well
                    self.response_diagonal_matrix = np.einsum("fii->fi", self.response_spectral_matrix)
                elif self.spectral_processing_parameters.requires_diagonal_spectral_response:
                    ...
                    if self.response_diagonal_matrix is None:
                        ...
                        self.response_diagonal_matrix = (
                            ...
                            * np.einsum("if,if->fi", response_fft, np.conj(response_fft))
                            + (1 - self.spectral_processing_parameters.exponential_averaging_coefficient) * self.response_diagonal_matrix
                        )
                if (
                    self.spectral_processing_parameters.requires_full_spectral_response
                    or self.spectral_processing_parameters.requires_diagonal_spectral_response
                    ...
                    if self.reference_spectral_matrix is None:
                        ...
                        self.reference_spectral_matrix = (
                            ...
                            * np.einsum("if,jf->fij", reference_fft, np.conj(reference_fft))
                            + (1 - self.spectral_processing_parameters.exponential_averaging_coefficient) * self.reference_spectral_matrix
                        )
                    # Get the diagonal matrix as well
                    self.reference_diagonal_matrix = np.einsum("fii->fi", self.reference_spectral_matrix)
                elif self.spectral_processing_parameters.requires_diagonal_spectral_reference:
                    ...
                    if self.reference_diagonal_matrix is None:
                        ...
                        self.reference_diagonal_matrix = (
                            ...
                            * np.einsum("if,if->fi", reference_fft, np.conj(reference_fft))
                            + (1 - self.spectral_processing_parameters.exponential_averaging_coefficient) * self.reference_diagonal_matrix
                        )
                if (
                    self.spectral_processing_parameters.requires_full_spectral_reference
                    or self.spectral_processing_parameters.requires_diagonal_spectral_reference
                ):
                    self.log("Computed Reference Spectral Matrix in " f"{time.time() - reference_spectral_time:0.2f} seconds")
                    ...
                    if self.response_reference_spectral_matrix is None:
                        ...
                        self.response_reference_spectral_matrix = (
                            ...
                            * np.einsum("if,jf->fij", response_fft, np.conj(reference_fft))
                            + (1 - self.spectral_processing_parameters.exponential_averaging_coefficient) * self.response_reference_spectral_matrix
                        )
                    self.log("Computed Crossspectral Matrix in " f"{time.time() - cross_spectral_time:0.2f} seconds")
                self.frames_computed += 1

            frames = self.frames_computed
        self.log(f"Computed Spectral Matrices for {frames} frames in " f"{time.time() - response_spectral_time:0.2f} seconds")
        gffpinv = None
        gfxpinv = None
...
def spectral_processing_process(
    ...
    spectral_processing_instance = SpectralProcessingProcess(
        (environment_name + " Spectral Processing Computation" if process_name is None else process_name),
        command_queue,
        data_in_queue,
        data_out_queue,
