...
from rattlesnake.process.sysid_data_analysis import SysIdMetadata, SysIdDataAnalysisUICommands, SysIdDataAnalysisCommands
...
class AbstractSysIdUI(AbstractUI):
    ...
    @abstractmethod
    def __init__(self, environment_type: ControlTypes, environment_name: str, rattlesnake: Rattlesnake):
        """
    ...
    def connect_sysid_callbacks(self):
        ...
        self.system_id_widget.preview_noise_button.clicked.connect(lambda: self.run_system_id(bool_preview=True, type="noise"))
        self.system_id_widget.preview_system_id_button.clicked.connect(lambda: self.run_system_id(bool_preview=True, type="transfer"))
        self.system_id_widget.start_button.clicked.connect(lambda: self.run_system_id(bool_preview=False, type="noise"))
    ...
    def get_sysid_metadata(self, hardware_metadata: HardwareMetadata):
        ...
        sysid_averaging_type = self.system_id_widget.averagingTypeComboBox.itemText(self.system_id_widget.averagingTypeComboBox.currentIndex())
        ...
        sysid_exponential_averaging_coefficient = self.system_id_widget.averagingCoefficientDoubleSpinBox.value()
        sysid_estimator = self.system_id_widget.estimatorComboBox.itemText(self.system_id_widget.estimatorComboBox.currentIndex())
        ...
        sysid_signal_type = self.system_id_widget.signalTypeComboBox.itemText(self.system_id_widget.signalTypeComboBox.currentIndex())
        sysid_window = self.system_id_widget.windowComboBox.itemText(self.system_id_widget.windowComboBox.currentIndex())
        sysid_overlap = self.system_id_widget.overlapDoubleSpinBox.value() / 100 if sysid_signal_type == "Random" else 0.0
        ...
        sysid_burst_ramp_fraction = self.system_id_widget.rampFractionDoubleSpinBox.value() / 100
        sysid_low_frequency_cutoff = self.system_id_widget.lowFreqCutoffSpinBox.value()
        sysid_high_frequency_cutoff = self.system_id_widget.highFreqCutoffSpinBox.value()
    ...
    def run_system_id(self, bool_preview, type):
        ...
        self.event_watcher.ready.connect(lambda: self.run_system_id_acquisition_noise(bool_preview=bool_preview, type=type))
    ...
    def update_sysid_plots(
        ...
        response_indices = [
            i for i in range(self.system_id_widget.response_selector.count()) if self.system_id_widget.response_selector.item(i).isSelected()
        ]
        reference_indices = [
            i for i in range(self.system_id_widget.reference_selector.count()) if self.system_id_widget.reference_selector.item(i).isSelected()
        ]
    ...
    def save_sysid_matrix_file(self):
        ...
        if file_filter == "NetCDF File (*.nc4)":
            netcdf_handle = nc4.Dataset(filename, "w", format="NETCDF4", clobber=True)  # pylint: disable=no-member
            # Create dimensions
            netcdf_handle.createDimension("response_channels", len(self.hardware_metadata.channel_list))
            ...
            netcdf_handle.time_per_write = self.hardware_metadata.samples_per_write / self.hardware_metadata.output_sample_rate
            netcdf_handle.time_per_read = self.hardware_metadata.samples_per_read / self.hardware_metadata.sample_rate
            netcdf_handle.hardware = self.hardware_metadata.hardware
            netcdf_handle.hardware_file = "None" if self.hardware_metadata.hardware_file is None else self.hardware_metadata.hardware_file
            ...
            field_dict["time_per_write"] = self.hardware_metadata.samples_per_write / self.hardware_metadata.output_sample_rate
            field_dict["time_per_read"] = self.hardware_metadata.samples_per_read / self.hardware_metadata.sample_rate
            field_dict["hardware"] = self.hardware_metadata.hardware
            field_dict["hardware_file"] = "None" if self.hardware_metadata.hardware_file is None else self.hardware_metadata.hardware_file
            ...
            for label, _ in labels:
                field_dict["channel_" + label] = np.array(
                    [("" if getattr(channel, label) is None else getattr(channel, label)) for channel in self.hardware_metadata.channel_list]
                )
            # print(field_dict)
    ...
    def load_sysid_matrix_file(self, filename, popup=True):
        ...
        if filename is None or filename == "":
            ...
            netcdf_handle = nc4.Dataset(filename, "r", format="NETCDF4")  # pylint: disable=no-member
